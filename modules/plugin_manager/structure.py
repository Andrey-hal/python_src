from random import randint
from modules.plugin_manager.main import plugins_init, make_update, wait_input
from modules.websocket_server.scheme import Update
import base64
import io
import matplotlib.pyplot as plt
from io import BytesIO
from urllib import parse
from json import dumps
from traceback import format_exc


class Plugin:
    __slots__ = ('widgets', 'id')
    name = 'default'
    description = ''
    author = 'default'
    screen_widgets = {}
    _handlers = {}
    client = None

    async def con_input(self,t):
        return await wait_input(t)

    @plugins_init
    def __init__(self):
        self.id = 'plugin_' + ''.join([str(randint(0, 10)) for i in range(10)])  # генератор id модуля

    async def create_widgets(self):
        done = [await i.draw() for n, i in self.screen_widgets.items()]
        return done

    def create_repr(self):
        return {
            'name': self.name,
            'description': self.description,
            'id': self.id,
            'author': self.author}

    def add_widget(self, *widget):
        for w in widget:
            self.screen_widgets[w.widget_id] = w
            w.widget_plugin = self

    def on_init(self, func):
        if 'on_init' not in self._handlers:
            self._handlers['on_init'] = [func, ]
        else:
            self._handlers['on_init'].append(func)

    def on_command(self, command):
        def wrap(func):
            if command not in self._handlers:
                self._handlers[command] = [func, ]
            else:
                self._handlers[command].append(func)

        return wrap

    def handle_command(self, command):
        args = command.split(' ')
        cmd = args[0]
        args = args[1:]
        if cmd in self._handlers:
            return self._handlers[cmd], args
        else:
            return None

    async def send_message(self, text):
        u = Update.CommandAnswer(command=text, plugin_id=self.id)
        await make_update(self.client, u)


class ScreenWidget:
    __slots__ = ('rendered', 'widget_id')
    widget_plugin = None
    widget_type = 'unknown'

    widget_styles = {}
    widget_rendered = {'widget_type': widget_type,
                       'widget_id': None,
                       'update_type': 'create_widget'}

    def set_layout(self, position=None, size=None):
        """

        :param position: {'x':0,'y':0}
        :type size: {'h':0,'w':0}
        """
        if position:
            self.widget_layout['position'] = position
        if size:
            self.widget_layout['size'] = size

    def set_style(self, **kwargs):
        prepared = {}
        for k, v in kwargs.items():
            k = k.replace('_', '-')
            prepared[k] = v
        self.widget_layout.update(prepared)

    def _layout_to_style(self):
        done = ''
        done += 'top:{}%;'.format(self.widget_layout['position']['y'])
        done += 'left:{}%;'.format(self.widget_layout['position']['x'])
        done += 'height:{}%;'.format(self.widget_layout['size']['h'])
        done += 'width:{}%;'.format(self.widget_layout['size']['w'])
        done += 'position:absolute;'
        return done

    def _dict_to_style(self):
        done = ';'.join(['{}:{}'.format(k, v) for k, v in self._layout_to_style().items()])
        return done

    async def draw(self, action='create_widget'):
        u = Update.Widget(update_type=action,
                          widget_type=self.widget_type,
                          widget_id=self.widget_id,
                          widget_style=self._layout_to_style())
        if self.widget_type == 'chart':
            u.add_type('chart')
            u.chart = self._config_structure
        elif self.widget_type == 'img':
            u.add_type('img')
            u.img = {'data': self.data}
        await make_update(self.widget_plugin.client, u)
        return u


class Chart(ScreenWidget):
    widget_layout = {
        'position': {'x': 10, 'y': 10},
        'size': {'h': 50, 'w': 50}
    }
    widget_type = 'chart'
    data = {}
    options = {}
    _config_structure = {
        'type': None,
        'options': {},
        'data': {}
    }

    def __init__(self, chart_type, name=None, options=None, data=None):
        if name is not None:
            self._config_structure['options']['title'] = {'display': True, 'text': name}
        self.type = chart_type
        self._config_structure['type'] = chart_type

        if options is not None:
            self._config_structure['options'].update(options)
            self.options = options
        if data:
            self.data = data
            self._config_structure['data'] = data

        self.widget_rendered[self.widget_type] = self._config_structure

        self.widget_id = 'chart_' + ''.join([str(randint(0, 10)) for i in range(10)])  # генератор id модуля

    async def update(self, data=None, options=None):
        if data:
            self.data = data
            self._config_structure['data'].update(data)
        if options:
            self.options = options
            self._config_structure['options'].update(options)

        self.widget_rendered[self.widget_type] = self._config_structure
        await self.draw('update_widget')

    async def redraw(self, chart_type=None, name=None, options=None, data=None):
        if chart_type:
            self.type = chart_type
            self._config_structure['type'] = chart_type
        if options:
            self.options = options
            self._config_structure['options'] = options
        if name:
            self.name = name
            self._config_structure['options']['title'] = {'display': True, 'text': name}
        if data:
            self.data = data
            self._config_structure['data'] = data
        self.widget_rendered[self.widget_type] = self._config_structure
        await self.draw('create_widget')


class Image(ScreenWidget):
    widget_layout = {
        'position': {'x': 10, 'y': 10},
        'size': {'h': 50, 'w': 50}
    }
    widget_type = 'img'
    data = 'data:image/png;base64,'

    def __init__(self, data=None):
        self.widget_id = 'img_' + ''.join([str(randint(0, 10)) for i in range(10)])  # генератор id модуля

        if data:
            self.data = 'data:image/png;base64,' + parse.quote(data)

    async def update(self, plot=None, img=None):

        if plot is not None:
            imgdata = BytesIO()
            plot.savefig(imgdata, format='png')
            imgdata.seek(0)
            self.data = 'data:image/png;base64,' + parse.quote(base64.b64encode(imgdata.read()))

        if img is not None:
            self.data = 'data:image/png;base64,' + parse.quote(img)
        await self.draw('update_widget')
